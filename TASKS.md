...
- [~] #18 doctor: name a trunk checkout holding an exact copy of a branch's diff @doctor !1 rework:"doctor fails on submodule/nested repo/non-UTF-8 untracked paths (copy check must never fail doctor); discard-copy deletes ignored files when a file becomes a directory (refuse parent/child path overlaps, check all before changing anything)" branch:doctor/task-18 submitted:ff03bad27f1e
...
- [~] #25 Per-subcommand --help prints that command's one-line usage, not the full global listing @output !1 needs:#13 branch:output/task-25 submitted:d32230be5716
