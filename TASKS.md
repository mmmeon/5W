...
- [~] #38 reject accepts a task that was never submitted: PROTOCOL.md allows reject only from [~] @queue !1 rework:"closed-task refusal still says 5w open <id> first, which now leads to a second refusal: say there is nothing to reject (or name a path that works) and test the closed case" branch:queue/task-38 submitted:cfd866abdd3e
