...
- [ ] #35 Stacked branches recorded with the trunk as parent: ship re-review and --sync conflicts on work already landed @wt !3 branch:wt/task-35 rework:"stack_notes makes doctor and wt ls ~400x slower (6s on 400 branches) and flags a parent as holding its own child, advising a parent cycle: detect in O(1) git processes and skip a held branch that records b up its parent chain or has no commits past b"
