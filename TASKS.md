...
- [ ] #52 ship prints 'shipping unreviewed' before its checks run, so a later refusal leaves a false first line @output !1 branch:output/task-52 rework:"notices still print before the fast-forward, which can itself refuse (uncommitted TASKS.md in main): print them only after the fast-forward succeeds; also defer verify_reviewed's --force lines; test both"
