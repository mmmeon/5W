...
- [ ] #19 Batch accept (accept 4 5 6) and ship --accepted: land every accepted branch bottom of stack first @queue !2 branch:queue/task-19 rework:"gate bypass: landed_under passes git's quoted/colon-prefixed path names back as pathspecs, so a parent adding a non-ASCII or ':' file counts as landed when it never did; use -z --no-renames and literal pathspecs, and test an accepted parent deleted without shipping"
