...
- [~] #35 Stacked branches recorded with the trunk as parent: ship re-review and --sync conflicts on work already landed @wt !3 rework:"stack_notes makes doctor and wt ls ~400x slower (6s on 400 branches) and flags a parent as holding its own child, advising a parent cycle: detect in O(1) git processes and skip a held branch that records b up its parent chain or has no commits past b" branch:wt/task-35 submitted:b1f5afd7b28b
...
- [~] #54 lint: a single-edit queue subject must name the one row the commit changes @queue !1 rework:"tool-made same-value set commits (text-only rewrite) fail the new check, so v0.1.x history is refused: allow the named row to be unchanged when parsed (only other rows may not change), make set of the same value say nothing changed, and have audit lint single-edit subjects too" branch:queue/task-54 submitted:1a640cbf1383
...
- [ ] #55 add mints ids from the working copy, so a later set of an uncommitted hand row commits an id lint calls reused @queue !1
