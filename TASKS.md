...
- [ ] #18 doctor: name a trunk checkout holding an exact copy of a branch's diff @doctor !1 branch:doctor/task-18 rework:"doctor fails on submodule/nested repo/non-UTF-8 untracked paths (copy check must never fail doctor); discard-copy deletes ignored files when a file becomes a directory (refuse parent/child path overlaps, check all before changing anything)"
