...
- [ ] #54 lint: a single-edit queue subject must name the one row the commit changes @queue !1 branch:queue/task-54 rework:"tool-made same-value set commits (text-only rewrite) fail the new check, so v0.1.x history is refused: allow the named row to be unchanged when parsed (only other rows may not change), make set of the same value say nothing changed, and have audit lint single-edit subjects too"
