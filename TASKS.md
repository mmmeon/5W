...
- [ ] #19 Batch accept (accept 4 5 6) and ship --accepted: land every accepted branch bottom of stack first @queue !2 branch:queue/task-19 rework:"change_id honours repo diff config: a textconv driver hid a post-review change and ship landed it; make change_id's diff immune to config (--no-textconv --no-ext-diff --unified=3 and the like) and test textconv plus a non-stacked submodule-only change"
