...
- [ ] #54 lint: a single-edit queue subject must name the one row the commit changes @queue !1 branch:queue/task-54 rework:"the new no-op check swallows real edits: a set leaves a working-copy hand edit on disk while saying 'already so', and no longer moves a misplaced row into its section; no-op only when committed rows, the row's section and the working/staged copies are all unchanged"
