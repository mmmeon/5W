...
- [ ] #31 README: one-line uninstall note (cargo uninstall 5w) next to the install line @output !1 needs:#13 branch:output/task-31 rework:"cargo uninstall 5w fails (package is fivew): use cargo uninstall fivew; add wt setup's git config and the worktrees root to what stays; fix the install comment that omits PROTOCOL.md"
