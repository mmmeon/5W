...
- [~] #22 JSON list rows: trim to id, state, level, area, title, branch by default; --full for the rest @output !2 needs:#13 rework:"short JSON row drops what the text row shows: keep unmet and rework in every list's default (blocked --json is meaningless without unmet); test blocked --json carries unmet; note the breaking change in README" branch:output/task-22 submitted:3ec5e7834a6d
...
- [~] #28 Color the '5w: <error>' refusal red when stderr is a terminal and NO_COLOR is unset @output !1 needs:#13 branch:output/task-28 submitted:5fb513e93e92
...
- [ ] #43 Unknown flags outside tasks.rs: wt ls, wt setup, report list and init accept them; lint reads --bogus as a revision @output !1 needs:#23
