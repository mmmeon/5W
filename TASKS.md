...
- [ ] #44 Refusals that append usage text after a newline: make each one line naming the fix @output !1 branch:output/task-44 rework:"three refusals are still multi-line (wt.rs:668 dirty wt rm, ship.rs:316 --sync rollback, upkeep.rs:41 check_requires) and the new debug_assert panics on them in debug builds: make them one line and have tests assert exactly one line and exit 1"
