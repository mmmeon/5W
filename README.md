...
- **`5w hook install`** (also run by `5w wt setup`) adds a pre-commit hook running `5w lint --staged`.
  ...
  to — as do the texts a brief prints, and `audit`'s doctor block — and `ship` and `wt prune`: the
  queue and archive they read, `commit_prefix`, `require_task` and `gate_trunk`. `5w doctor` reads
  the checkout as it stands. While the trunk checkout's copy differs from the committed one, these
  commands say so (on stderr, or on a refusal's line) and `doctor` notes it; a queue file only that
  copy names is refused with the fix, committing `.5w.toml`, not `init`. A trunk that commits none,
  or one that does not parse, is read under the checkout's.
...
server is its admin's step. `5w hook install pre-receive` records `5w.trunk` when it is unset or empty (a re-install too), from
`FIVEW_TRUNK` when that is set, else from `HEAD` —
...
(committed so, even when the checkout's copy is fixed),
...
`ship`, `wt prune` and the hook once it is committed on the trunk: until then they read the committed copy and say
