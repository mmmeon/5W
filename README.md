...
- **`5w hook install`** (also run by `5w wt setup`) adds a pre-commit hook running `5w lint --staged`.
  ...
  to — as do the texts a brief prints, and `audit`'s doctor block — and `ship` and the `wt`
  commands: the queue and archive they read, `commit_prefix`, `require_task`, `gate_trunk` and the
  `worktrees` settings (`wt path` reads none). `5w doctor` reads
